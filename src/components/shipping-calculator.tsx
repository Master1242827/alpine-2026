...
import { Truck, Loader2, MapPin, HelpCircle, Clock, BadgeCheck, PackageCheck } from "lucide-react";
...
const CEP_STORAGE_KEY = "alpine.lastCep";

export function ShippingCalculator({ items, compact = false, freeShippingThresholdCents }: Props) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) setCep(formatCep(saved));
    } catch {
      /* ignore */
    }
  }, []);

  // Recalculate automatically when items change (e.g., variation switch), if a CEP was already calculated
  useEffect(() => {
  ...
  async function handleCalc(silent = false) {
    ...
    try {
      try {
        localStorage.setItem(CEP_STORAGE_KEY, clean);
      } catch {
        /* ignore */
      }
        ...
        <ul className="mt-3 space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
          {options.map((o, idx) => {
            ...
            return (
              <li
                key={o.id}
                className="group relative flex items-center justify-between gap-3 rounded-xl border border-border bg-background p-3 text-sm transition-all hover:border-primary/40 hover:shadow-md"
                style={{ animationDelay: `${idx * 40}ms` }}
              >
                <div className="flex min-w-0 items-center gap-3">
                  {o.companyPicture ? (
                    <img
                      src={o.companyPicture}
                      alt=""
                  ...
                  <div className="min-w-0">
                    <p className="flex items-center gap-1.5 truncate font-semibold">
                      ...
                      {isFastest && (
                        <span className="rounded-full bg-blue-500/10 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-blue-600 dark:text-blue-400">
                          Mais rápido
                        </span>
                      )}
                    </p>
                    {o.deliveryDays != null && (
                      <p className="mt-0.5 flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        Em até {o.deliveryDays} dia{o.deliveryDays === 1 ? "" : "s"} útil
                        {o.deliveryDays === 1 ? "" : "eis"}
                      </p>
                    )}
                  </div>
                </div>
